...
class ParticipantDataService extends DataService<ParticipantDTO> {
  ...
  public async getDisciplines(id: number): Promise<Discipline[]> {
    const response = await fetch(this.endpoint + this.uri + `/${id}/disciplines`);
    if (!response.ok) {
  ...
  public async addDiscipline(id: number, disciplineId: number): Promise<Discipline[]> {
    const response = await fetch(this.endpoint + this.uri + `/${id}/disciplines`, {
      method: "PUT",
      headers: {
  ...
  public async removeDiscipline(id: number, disciplineId: number): Promise<Discipline[]> {
    const response = await fetch(this.endpoint + this.uri + `/${id}/disciplines/${disciplineId}`, {
      method: "DELETE"
    });
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}, ${await response.text()}`);

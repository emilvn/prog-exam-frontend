        ...
        <>
            {sortedResults.length > 0 && (
                <div>
                    <h2
                        className={
                            "bg-sky-300 p-2 font-semibold text-white text-xl px-8"
                        }
                    >
                        {discipline.name}
                    </h2>
                                ...
                                .map((result) => {
                                    ...
                                    return (
                                        <tr key={result.id}>
                                            ...
                                            <td>
                                                {participant?.birthDate.getFullYear()}
                                            </td>
...
function Home() {
    ...
    useEffect(() => {
        if (selectedDiscipline) {
            setFilteredResults(
                results.filter(
                    (result) => result.disciplineId === selectedDiscipline.id
                )
            );
        } else {
    ...
    useEffect(() => {
        if (selectedGender === "M") {
            setFilteredParticipants(
                participants.filter((participant) => participant.isMale)
            );
        ...
        if (selectedAgeGroup !== "") {
            setFilteredParticipants(
                participants.filter((participant) => {
                    ...
                    if (selectedAgeGroup === "U15") {
                        return age < 15;
                    } else if (selectedAgeGroup === "U17") {
                        return age >= 15 && age < 17;
                    } else if (selectedAgeGroup === "U19") {
                        return age >= 17 && age < 19;
                    } else if (selectedAgeGroup === "U21") {
                        return age >= 19 && age < 21;
